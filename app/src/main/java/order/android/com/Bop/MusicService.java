...
public class MusicService extends Service {
     ...
     * @param sourceType
     */
    private void addToPlayList(final long[] list, int position, long sourceId, BopUtil.IdType sourceType) {
...
//                    mPlayPos = pos;
    ...
    private void doAutoShuffleUpdate() {
        ...
        for (int i = 0; i < toAdd; i++) {
            ...
            mPlaylist.add(new MusicPlaybackTrack(mAutoShuffleList[idx], -1, BopUtil.IdType.NA, -1));
            notify = true;
        }
        if (notify) {
            notifyChange(QUEUE_CHANGED);
    ...
    private void updateMediaSession(final String what) {
        ...
        if (what.equals(PLAYSTATE_CHANGED) || what.equals(POSITION_CHANGED)) { //播放状态改变时
            ...
            Bitmap albumArt = ImageLoader.getInstance().loadImageSync(BopUtil.getAlbumArtUri(getAlbumId()).toString());
            if (albumArt != null) {
    ...
    private Notification buildNotification() {
        ...
        Bitmap artwork;
        artwork = ImageLoader.getInstance().loadImageSync(BopUtil.getAlbumArtUri(getAlbumId()).toString());

        if (artwork == null) {
            artwork = ImageLoader.getInstance().loadImageSync("drawable://" + R.drawable.defult_album_art);
        }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            NotificationChannel notificationChannel = new NotificationChannel(NOTIFICATION_CHANNEL_ID, "My Notifications",NotificationManager.INTERRUPTION_FILTER_UNKNOWN);
        ...
        notificationBuilder
                ...
                .addAction(R.drawable.ic_fast_forward_white_24dp,
                        "",
                ...
                try {
                    if (mCursor != null && shouldAddToPlaylist) {
                        ...
                        mPlaylist.add(new MusicPlaybackTrack(
                                mCursor.getLong(IDCOLIDX), -1, BopUtil.IdType.NA, -1));
                        notifyChange(QUEUE_CHANGED);
                        mPlayPos = 0;
                        mHistory.clear();
                    }
     ...
     * @param sourceType
     */
    public void open(final long[] list, final int position, long sourceId, BopUtil.IdType sourceType) {
        synchronized (this) {
            if (mShuffleMode == SHUFFLE_AUTO) {
     ...
     * @param sourceType
     */
    public void enqueue(final long[] list, final int action, long sourceId, BopUtil.IdType sourceType) {
        synchronized (this) {
    ...
    private static final class ServiceStub extends IListenerService.Stub {
        ...
        public void open(final long[] list, final int position, long sourceId, int sourceType)
            ...
            mService.get().open(list, position, sourceId, BopUtil.IdType.getTypeById(sourceType));
        }
        ...
        public void enqueue(final long[] list, final int action, long sourceId, int sourceType)
                throws RemoteException {
            mService.get().enqueue(list, action, sourceId, BopUtil.IdType.getTypeById(sourceType));
        }
