...
public class ArtistSongAdapter extends RecyclerView.Adapter<ArtistSongAdapter.ItemHolder>{
    ...
    public ItemHolder onCreateViewHolder(ViewGroup viewGroup, int viewType) {
        if (viewType == 0) {
            ...
            View v2 = LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.recyceler_song, viewGroup, false);
                    ...
                    public boolean onMenuItemClick(MenuItem item) {
                        switch (item.getItemId()) {
                            ...
                            case R.id.popup_song_addto_playlist:
                                BopUtil.showAddPlaylistDialog(mContext,new long[]{arraylist.get(position + 1).id});
                                break;
                            ...
                            case R.id.popup_song_share:
                                BopUtil.shareTrack(mContext, arraylist.get(position + 1).id);
                                break;
                            case R.id.popup_song_delete:
                                long[] deleteIds = {arraylist.get(position + 1).id};
                                BopUtil.showDeleteDialog(mContext, arraylist.get(position + 1).title, deleteIds,
    ...
    public class ItemHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        public void onClick(View v) {
            ...
            handler.postDelayed(new Runnable() {
                @Override
                public void run() {
                    MusicPlayer.playAll(mContext, songIDs, getAdapterPosition() - 1, artistID, BopUtil.IdType.Artist, false);
                }
            }, 100);
        }
